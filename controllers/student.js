...
// Method to render the assignment submission form
exports.submitAssignmentPage = async (req, res, next) => {
  try {
    ...
    const sql = `
      ...
      FROM 
        assignment
      INNER JOIN 
        class ON assignment.class_id = class.class_id
      INNER JOIN 
        student ON student.section = class.section
      WHERE 
        student.s_id = ? 
        AND SUBSTRING(student.dept_id, 1, 2) = SUBSTRING(class.c_id, 1, 2);
    `;

    const assignments = await queryParamPromise(sql, [studentId]);

    res.render('Student/submitAssignments', {
      assignments,
      page_name: 'submit-assignments',
    });
  } catch (error) {
    console.error('Error fetching assignments for submission:', error);
    res.status(500).send('Internal Server Error');
  }
};


// Method to handle the assignment submission
exports.submitAssignment = async (req, res, next) => {
  const { asg_id } = req.body; 
  const studentId = req.user; 
  if (!asg_id) {
  ...
  const sql = 'INSERT INTO assignment_submission (s_id, asg_id) VALUES (?, ?)';
  try {
    await queryParamPromise(sql, [studentId, asg_id]);
    res.redirect('/student/view-submissions'); 
  } catch (error) {
    if (error.code === 'ER_DUP_ENTRY') { 
      res.status(500).send('Duplicate Entry Found');
    } else {
...
exports.viewSubmissions = async (req, res, next) => {
  ...
  try {
    // Updated query to fetch submissions and assignment details
    const sql = `
      SELECT 
        assignment.name,
        assignment.deadline,
        assignment.day AS submission_date
      FROM 
        assignment
      INNER JOIN 
        assignment_submission ON assignment.asg_id = assignment_submission.asg_id
      WHERE 
        assignment_submission.s_id = ?
    `;

    const submissions = await queryParamPromise(sql, [studentId]);

    res.render('Student/viewSubmissions', {
      submissions,
      page_name: 'view-submissions',
    });
  } catch (error) {
...
exports.getFees = async (req, res, next) => {
  try {
      ...
      const feesData = await queryParamPromise('SELECT * FROM fee WHERE s_id = ?', [studentData.s_id]);

      // Convert amounts to numbers before summing
      const paidFees = feesData.reduce((sum, fee) => sum + parseFloat(fee.amount), 0);

      const totalFeesToBePaid = 50000; // Example total fee
      const outstandingBalance = totalFeesToBePaid - paidFees;

      res.render('Student/fee', {
          page_name: 'fee',
          studentData,
          feesData,
          paidFees,
      ...
      res.status(500).send('An error occurred while fetching fees data.');
  }
};
